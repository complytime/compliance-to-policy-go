...
// LaunchAggregatorPlugins launches requested policy aggregation plugins and configures each plugin to make it ready for use with defined plugin workflows.
// The plugin is configured based on default options and given options.
// Given options are represented by config.PluginConfig.
func (m *PluginManager) LaunchAggregatorPlugins(manifests plugin.Manifests, pluginConfig PluginConfig) (map[plugin.ID]policy.Aggregator, error) {
	pluginsByIds := make(map[plugin.ID]policy.Aggregator)
	for _, manifest := range manifests {
		policyPlugin, err := plugin.NewAggregatorPlugin(manifest, m.clientFactory)
		if err != nil {
...
// LaunchGeneratorPlugins launches requested policy generation plugins and configures each plugin to make it ready for use with defined plugin workflows.
// The plugin is configured based on default options and given options.
// Given options are represented by config.PluginConfig.
func (m *PluginManager) LaunchGeneratorPlugins(manifests plugin.Manifests, pluginConfig PluginConfig) (map[plugin.ID]policy.Generator, error) {
	pluginsByIds := make(map[plugin.ID]policy.Generator)
	for _, manifest := range manifests {
