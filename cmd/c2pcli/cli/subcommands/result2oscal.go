...
func runResult2Policy(ctx context.Context, option *Options) error {
	...
	foundPlugins, err := manager.FindRequestedPlugins(inputContext.RequestedProviders(), plugin.AggregationPluginName)
	if err != nil {
		return err
	}
	...
	launchedPlugins, err := manager.LaunchAggregatorPlugins(foundPlugins, configSelections)
	if err != nil {
		return err
	}
	defer manager.Clean()
